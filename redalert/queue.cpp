...
static void Queue_Record(void)
{
    ...
    //------------------------------------------------------------------------
    Compute_Game_CRC();
     Session.RecordFile.Write(&GameCRC, sizeof(GameCRC));
...
static void Queue_Playback(void)
{
   ...
   /* testframe = ((Frame + (Session.FrameSendRate - 1)) / Session.FrameSendRate) * Session.FrameSendRate;
    ...
    unsigned long readGameCRC;
    Session.RecordFile.Read(&readGameCRC, sizeof(GameCRC));
    Session.RecordFileCurrentReadPos += sizeof(GameCRC);
    Compute_Game_CRC();
    if (readGameCRC != GameCRC) {
        Print_CRCs(NULL);
        GameActive = 0;
    }
